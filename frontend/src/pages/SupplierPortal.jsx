          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E8EAED', padding: 24, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {phases.map((ph, i) => {
                ...
                return (
                  <div key={ph.id || i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 14px', borderRadius: 10, background: isActive ? '#FFF8F5' : '#FAFAFA', border: `1px solid ${isActive ? BRAND + '40' : '#F0F2F4'}` }}>
                    <div style={{ width: 20, height: 20, borderRadius: '50%', background: isDone ? '#22c55e' : isActive ? BRAND : '#E5E7EB', display: 'grid', placeItems: 'center', flexShrink: 0, marginTop: 1 }}>
                      {isDone ? <span style={{ fontSize: 11, color: '#fff' }}>✓</span> : <span style={{ fontSize: 10, color: isActive ? '#fff' : '#9CA3AF', fontWeight: 700 }}>{i + 1}</span>}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: '#15171C', margin: 0 }}>{ph.name}</p>
                      {ph.start_date && ph.end_date && (
                        <p style={{ fontSize: 11, color: '#9CA3AF', margin: '2px 0 0' }}>
                          {new Date(ph.start_date).toLocaleDateString('fr-CA', { day: 'numeric', month: 'short' })} → {new Date(ph.end_date).toLocaleDateString('fr-CA', { day: 'numeric', month: 'short' })}
                        </p>
                      )}
                    </div>
