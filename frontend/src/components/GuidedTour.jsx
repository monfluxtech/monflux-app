...
const STEPS = [
  {
    ...
    desc: 'Cette visite guidée te montre les sections principales de l\'application en moins de 2 minutes. Tu peux la relancer à tout moment depuis Paramètres → Profil.',
    position: 'center',
  },
  {
    target: '.app-sidebar',
    title: '🧭 La barre de navigation',
    desc: 'À gauche : tu trouves toutes les sections de l\'app. Le menu s\'adapte selon ton forfait et tes modules activés.',
    position: 'right',
  },
  {
    target: '[href="/projets"], [href*="/projets"]',
    title: '📁 Projets',
  ...
  {
    target: '[href="/"], [href*="/tableau"]',
    title: '📊 Tableau de bord',
    desc: 'Ton aperçu financier : valeur portefeuille, facturé, marges, délais. Les chiffres se mettent à jour en temps réel.',
    position: 'right',
  },
  {
    target: '[href="/flo"], [href*="florence"], .flo-btn, [aria-label*="Flo"], [aria-label*="Florence"]',
    title: '✨ Florence — ton IA de chantier',
    desc: 'Flo analyse tes projets, génère des soumissions, trouve des sous-traitants, rédige des courriels et bien plus. Le bouton orange en bas à droite la contacte directement.',
    position: 'right',
  },
  {
    target: null,
...
export default function GuidedTour() {
  ...
  const stop = () => { setStep(-1); localStorage.setItem('mf_tour_done', '1'); };

  useEffect(() => {
    const handler = () => start();
    window.addEventListener('mf:start-tour', handler);
    // Auto-launch on first visit
    if (!localStorage.getItem('mf_tour_done') && !localStorage.getItem('mf_tour_step')) {
  ...
  const popupStyle = isCentered
    ...
    : (() => {
        const top = Math.max(8, Math.min(targetRect.top, window.innerHeight - 220));
        return { position: 'fixed', top, left: targetRect.right + 16, zIndex: 9999 };
      })();
    ...
    <>
      ...
      <div ref={overlayRef} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button
            onClick={() => step > 0 ? setStep(s => s - 1) : stop()}
