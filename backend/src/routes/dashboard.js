...
router.get('/summary', async (req, res) => {
  ...
  try {
    const [projects, invoices, timesheets] = await Promise.all([
      ...
      query(`SELECT hours_total FROM timesheets WHERE company_id = $1 AND clock_out IS NOT NULL`, [cid]),
    ]);

    const TERMINAL = ['clos', 'paye'];
    const ACTIVE_SITES = ['en_chantier'];
    const PRE_SALE = ['brouillon','estimation','prix_envoye'];
    const ACCEPTED = ['accepte','planifie','en_chantier','a_facturer'];

    const active_projects   = projects.rows.filter(p => ACTIVE_SITES.includes(p.status)).length;
    const to_invoice        = projects.rows.filter(p => p.status === 'a_facturer').length;
    const pipeline_projects = projects.rows.filter(p => PRE_SALE.includes(p.status)).length;
    const pipeline_value    = projects.rows
      .filter(p => !TERMINAL.includes(p.status))
      .reduce((s,p) => s + Number(p.contract_value||0), 0);
    const contracted_value  = projects.rows
      .filter(p => ACCEPTED.includes(p.status))
      .reduce((s,p) => s + Number(p.contract_value||0), 0);
    ...
    res.json({
      active_projects, to_invoice, pipeline_projects,
      pipeline_value, contracted_value,
      total_revenue, outstanding, overdue_count, total_hours,
    });
  } catch (err) {
    console.error(err);
